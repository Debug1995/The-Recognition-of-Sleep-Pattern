...
int SVM(float x_test[136], int index){
	// time stamp
	...
	FILE *f_pca_coef;
	FILE *f_pca_mean;
	f_pca_coef = fopen("./models/pca_coef", "r");
	f_pca_mean = fopen("./models/pca_mean", "r");	
	...
	if(index == 0)
	{
		f_sv_sigma = fopen("./models/sv_sigma", "r");
		f_sv_mean = fopen("./models/sv_mean", "r");
	}
	else
	{
		sprintf(path, "./models/sv_sigma%d", index);
		f_sv_sigma = fopen(path, "r");
		sprintf(path, "./models/sv_mean%d", index);
	...
	if(index == 0)
	{
		f_sv = fopen("./models/sv", "r");
		f_sv_labels = fopen("./models/sv_labels", "r");
		f_sv_alphas = fopen("./models/sv_alphas", "r");
		f_sv_bias = fopen("./models/sv_bias", "r");
		f_sv_lambda = fopen("./models/sv_lambda", "r");
	}
	else
	{
		sprintf(path, "./models/sv%d", index);
		f_sv = fopen(path, "r");
		sprintf(path, "./models/sv_labels%d", index);
		f_sv_labels = fopen(path, "r");
		sprintf(path, "./models/sv_alphas%d", index);
		f_sv_alphas = fopen(path, "r");	
		sprintf(path, "./models/sv_bias%d", index);
		f_sv_bias = fopen(path, "r");
		sprintf(path, "./models/sv_lambda%d", index);
